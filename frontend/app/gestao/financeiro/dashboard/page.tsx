...
export default function FinanceiroDashboardPage() {
  ...
  if (loading) {
    return <div className="p-6 text-sm text-slate-500">Carregando dashboard financeiro...</div>;
  }
  ...
  return (
    <div className="space-y-6 pb-10">
      <header>
        <h1 className="text-2xl font-semibold text-slate-900">Gestao Financeira</h1>
        <p className="mt-1 text-sm text-slate-600">Visao executiva de caixa, recebimentos, pagamentos e alertas operacionais.</p>
      </header>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="rounded-xl border border-slate-200 bg-white p-4 xl:col-span-2">
          <h2 className="text-sm font-semibold text-slate-900">Fluxo de caixa (ultimos 30 dias)</h2>
          <div className="mt-4 space-y-3">
            {fluxoSerie.map((item: any) => (
              <div key={item.data} className="grid grid-cols-[90px_1fr_1fr] items-center gap-3 text-xs">
                ...
                <div className="h-2 rounded bg-rose-50">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-900">Recebimentos por categoria</h2>
          <ul className="mt-3 space-y-2 text-xs">
            {(data?.charts?.recebimentosPorCategoria || []).map((item: any) => (
              <li key={item.categoria} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
          ...
          <h3 className="mt-4 text-sm font-semibold text-slate-900">Pagamentos por categoria</h3>
          <ul className="mt-3 space-y-2 text-xs">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-900">Faturas vencendo hoje</h2>
          <ul className="mt-3 space-y-2 text-xs">
            {(data?.listasRapidas?.vencendoHoje || []).map((item: any) => (
              <li key={item.id} className="rounded-lg bg-slate-50 px-3 py-2">
                ...
                <p className="mt-1 text-slate-600">{item.cliente}</p>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-900">Faturas em atraso</h2>
          <ul className="mt-3 space-y-2 text-xs">
            {(data?.listasRapidas?.emAtraso || []).map((item: any) => (
              <li key={item.id} className="rounded-lg bg-rose-50 px-3 py-2">
                <div className="flex items-center justify-between">
                  <strong>{item.codigoFatura}</strong>
                  <span className="text-rose-700">{item.diasAtraso} dias</span>
                </div>
                <p className="mt-1 text-slate-700">{toCurrency(item.saldoAberto)}</p>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-900">Ultimos recebimentos</h2>
          <ul className="mt-3 space-y-2 text-xs">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-900">Ultimos pagamentos</h2>
          <ul className="mt-3 space-y-2 text-xs">
